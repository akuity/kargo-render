...
func TestServerConfig(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "BOOKKEEPER_TLS_ENABLED not a bool",
			setup: func() {
				t.Setenv("PORT", "8080")
				t.Setenv("BOOKKEEPER_TLS_ENABLED", "nope")
			},
			assertions: func(_ http.ServerConfig, err error) {
				require.Error(t, err)
				require.Contains(t, err.Error(), "was not parsable as a bool")
				require.Contains(t, err.Error(), "BOOKKEEPER_TLS_ENABLED")
			},
		},
		{
			name: "BOOKKEEPER_TLS_CERT_PATH required but not set",
			setup: func() {
				t.Setenv("BOOKKEEPER_TLS_ENABLED", "true")
			},
			assertions: func(_ http.ServerConfig, err error) {
				require.Error(t, err)
				require.Contains(t, err.Error(), "value not found for")
				require.Contains(t, err.Error(), "BOOKKEEPER_TLS_CERT_PATH")
			},
		},
		{
			name: "BOOKKEEPER_TLS_CERT_PATH required but not set",
			setup: func() {
				t.Setenv("BOOKKEEPER_TLS_CERT_PATH", "/var/ssl/cert")
			},
			assertions: func(_ http.ServerConfig, err error) {
				require.Error(t, err)
				require.Contains(t, err.Error(), "value not found for")
				require.Contains(t, err.Error(), "BOOKKEEPER_TLS_KEY_PATH")
			},
		},
		{
			name: "success",
			setup: func() {
				t.Setenv("BOOKKEEPER_TLS_KEY_PATH", "/var/ssl/key")
			},
